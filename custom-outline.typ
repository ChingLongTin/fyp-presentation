#let current-heading(level: auto, outlined: true) = {
  let here = here()
  let sel = if level == auto { heading.where(outlined: outlined) } else { heading.where(level: level, outlined: outlined) }
  query(sel.before(inclusive: false, here)).at(-1, default: none)
}

#let headings-context(target: heading.where(outlined: true), scope-level: auto) = {
  let current-heading = current-heading(level: scope-level)
...
#let custom-outline(
  ...
  let cx = headings-context(target: target, scope-level: scope-level)
  let idx = cx.at("current-heading-idx", default: none)
  let scope-path = if idx != none { cx.headings.at(idx).path }

  // Map headings to enriched dicts with order and here-order
  let enriched = ()
  let order = -1
  for hd in cx.headings {
    order = order + 1
  ...
  let headings = enriched.filter(filter)
